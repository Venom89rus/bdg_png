...
import pandas as pd                            # Работа с таблицами, фильтрация, группировка, чтение Excel-файлов и т.п.
import numpy as np                             # Математические операции, массивы, генерация чисел
import matplotlib.pyplot as plt                # Построение графиков, диаграмм и визуализация данных
from CoolProp.HumidAirProp import HAPropsSI    # Расчет свойств воздуха и водяного пара (для определения содержания влаги в газе).
import math                                    # Базовые математические операции, которые не входят в стандарт Python напрямую.
from io import BytesIO                         # Временное хранение файла в памяти (например, Excel-документа), который потом предлагается пользователю для скачивания.
import requests                                # Взаимодействие с веб-сервисами или загрузка данных из сети (Позволяет скачивать данные из интернета, отправлять запросы к API и т.д.)
import xlsxwriter                              # Используется как движок для записи Excel-файлов в сочетании с pd.ExcelWriter.
...
weather_api_key = st.sidebar.text_input("Введите API ключ OpenWeatherMap", type="password") # type="password" означает, что текст будет скрыт (как пароль).
location = st.sidebar.text_input("Введите название населённого пункта", value="Муравленко") # Ввод города или необходимых координат, поумолчанию Муравленко
...
if weather_api_key and location: # Проверка: если пользователь ввёл и API-ключ, и название города, тогда продолжаем выполнение.
    try:
        ...
        if "main" in weather_data:               # Проверка, есть ли в ответе секция "main" (где хранятся температурные данные)
            ground_temp = weather_data["main"]["temp"] - 3  # Берётся температура воздуха (weather_data["main"]["temp"]) и от неё вычитается 3°C, чтобы приблизительно оценить температуру грунта (так как она обычно немного ниже температуры воздуха).
            st.sidebar.success(f"Температура грунта: {ground_temp:.1f} °C") # Если всё прошло успешно, отображается результат в сайдбаре — температура грунта, округлённая до одного знака после запятой.
    except Exception as e:
        st.sidebar.error("Ошибка при получении данных о погоде")

# --- Загрузка данных Excel ---
uploaded_file = st.sidebar.file_uploader("Загрузите Excel файл с данными", type="xlsx") # Размещаем в боковой панели slidebar
if uploaded_file:                             # Проверяем загружен ли файл. Если файл действительно загружен (то есть uploaded_file не None), тогда выполняется следующий блок кода.
    excel_data = pd.ExcelFile(uploaded_file)  # Загружает Excel-файл в оперативную память и позволяет работать с ним без немедленного считывания всех листов.

    df = excel_data.parse('dhtmlxGrid') # Загружает в память конкретный лист Excel-файла с названием dhtmlxGrid, результат сохраняется в переменной df, которая становится таблицей данных

    if 'Дата протокола' in df.columns:      # Проверяет, есть ли в таблице столбец с названием Дата протокола
        df['Дата протокола'] = pd.to_datetime(df['Дата протокола'], errors='coerce') # Приводит значения этого столбца к формату даты и времени, т.е. преобразует строки или числа в даты

    # --- Выбор Месторождения, ДНС и ступени ---
    st.sidebar.subheader("Выбор параметров")
    field = st.sidebar.selectbox("Выберите месторождение", df['Месторождение'].unique()) # Создаёт выпадающий список в сайдбаре с надписью: "Выберите месторождение". df['Месторождение'].unique() — извлекает список уникальных месторождений из исходного DataFrame df
    df_field = df[df['Месторождение'] == field]                                          # Отфильтровывает DataFrame, оставляя только строки, соответствующие выбранному месторождению

    dns = st.sidebar.selectbox("Выберите объект подготовки (ДНС)", df_field['ДНС'].unique()) # Создаёт второй выпадающий список. Извлекает уникальные значения ДНС, только среди строк выбранного месторождения (df_field)
    df_dns = df_field[df_field['ДНС'] == dns]                                                # Ещё одно фильтрование: оставляет только те строки, где объект подготовки соответствует выбранному (dns)

    stage = st.sidebar.selectbox("Укажите ступень отбора", df_dns['Ступень отбора'].unique()) # Аналогично выбору объекта подготовки
    selected_df = df_dns[df_dns['Ступень отбора'] == stage]

    selected_row = selected_df.iloc[-1]  # Берётся последняя строка из selected_df — предположительно, самая последняя (актуальная) запись. Потому что, как правило, данные хранятся в хронологическом порядке.
    ...
    gas_flow = st.sidebar.number_input("Расход газа (м³/сут)", min_value=1.0, value=100000.0)  # Расход задается в ручную
    ...
    composition = {k: v / total_mol for k, v in composition.items()}  # Все значения из словаря composition делятся на total_mol, чтобы привести их к долям от 1.0
    ...
    effective_temp = min(gas_temp, ground_temp if ground_temp is not None else gas_temp) # Выбирается наименьшее значение между температурой газа и температурой грунта, используем минимальную температуру, поскольку именно при ней возможна конденсация воды
    ...
    dew_mode = st.sidebar.radio("Источник данных о воде", ["Измеренное содержание (г/м³)", "По точке росы (°C)"]) # Пользователю предлагается выбрать, откуда будет вводиться информация о влажности
    if dew_mode == "Измеренное содержание (г/м³)":
        ...
        dew_point = st.sidebar.number_input("Точка росы (°C)", value=0.0)  # Расчет по точке росы, то пользователь вводит её значение (например, 0 °C)
        try:
            RH = 1.0                                     # Выполняется расчет концентрации воды с помощью библиотеки CoolProp,
            w_kg_kgdryair = HAPropsSI("W", "T", dew_point + 273.15, "P", P_Pa, "R", RH)  # HAPropsSI(...) — функция расчета влажности воздуха по параметрам
    ...
    try:
        max_w_kg_kgdryair = HAPropsSI("W", "T", T_K, "P", P_Pa, "R", 1.0)  # Вычисляется максимальное влагосодержание при текущей температуре и давлении (100% влажность), предельно допустимая концентрация водяного пара в газе, выше которой начинается конденсация (конденсат в трубопроводе).
    ...
    if max_water_g_m3 is not None:                                         # Если расчет допустимой влаги выполнен (max_water_g_m3 не None) и фактическое содержание воды превышает допустимое, начинаем расчет
        if measured_water_content > max_water_g_m3:
            ...
            excess_water_kg = (excess_water / 1000) * gas_flow * gas_density # Перевод избытка влаги в массу в килограммах: excess_water / 1000 — в кг/м³, gas_flow — объем газа в м³/сут, gas_density — масса 1 м³ газа
            ...
            result["Избыток влаги (г/м³)"] = excess_water                      # Все расчеты добавляются в словарь result, затем он выводится в таблицу
            ...
            st.success("💧 Требуется подача метанола")                          # Преобразуем словарь result в таблицу и выводим в интерфейсе.
        else:
    ...
    st.subheader("📊 График расхода метанола")               # Если метанол рассчитан: Строится столбчатая диаграмма. Сравниваются: фактический расчёт и оптимальный (экономичный) расход.
